...
@Component({
    ...
    templateUrl: './dashboard-consultation-prescription.component.html',
    styleUrls: ['./dashboard-consultation-prescription.component.css']
})
export class DashboardConsultationPrescriptionComponent implements OnInit {
...
// for (var i = 0; i < followUpArray.length; i++) {
    //     if(this.maximumFollowUpDays < followUpArray[i]){
        ...
        var newMaxValue = followUpCount.findIndex(x => x == Math.max(...followUpCount));
        ...
        if(data.response){
            ...
            for (var k = 0; k < data.dataInvestigationPrescribed.length; k++) {
                for (var l = 0;l < data.dataInvestigationPrescribed[k].precription.investigationArray.length; l++) { 
                        ...
                        for (var m = 0; m <  data.dataInvestigationPrescribed[k].precription.investigationArray[l].values.length; m++) {                     
                            ...
                            for (var a = 0; a < this.investigationUniqueArray.length; a++) {
                                if(data.dataInvestigationPrescribed[k].precription.investigationArray[l].name == this.investigationUniqueArray[a].type &&
                                    data.dataInvestigationPrescribed[k].precription.investigationArray[l].values[m].value == this.investigationUniqueArray[a].name){
                                    boolean = true
                                break;
                            }
                        }
                    ...
                    for (var a = 0; a < this.investigationUniqueArray.length; a++) {
                        if(data.dataInvestigationPrescribed[k].precription.investigationArray[l].name == this.investigationUniqueArray[a].type &&
                            data.dataInvestigationPrescribed[k].precription.investigationArray[l].values == this.investigationUniqueArray[a].name){
                            boolean1 = true
                        break;
                    }
                }
                ...
                if(data.dataInvestigationPrescribed[k].precription.investigationArray[l].name !='Mammography' && 
                    ...
                    for (var m = 0; m <  data.dataInvestigationPrescribed[k].precription.investigationArray[l].values.length; m++) {
                        if(data.dataInvestigationPrescribed[k].precription.investigationArray[l].name == this.investigationUniqueArray[i].type &&
                            data.dataInvestigationPrescribed[k].precription.investigationArray[l].values[m].value == this.investigationUniqueArray[i].name){
                            this.investigationUniqueArray[i].count++
                    }
                }
            }else{
                if(data.dataInvestigationPrescribed[k].precription.investigationArray[l].name == this.investigationUniqueArray[i].type &&
                    data.dataInvestigationPrescribed[k].precription.investigationArray[l].values == this.investigationUniqueArray[i].name){
                    this.investigationUniqueArray[i].count++
            }
        }
    }

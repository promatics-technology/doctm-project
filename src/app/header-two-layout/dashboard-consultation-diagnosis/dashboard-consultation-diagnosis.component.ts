...
@Component({
    ...
    templateUrl: './dashboard-consultation-diagnosis.component.html',
    styleUrls: ['./dashboard-consultation-diagnosis.component.css']
})
export class DashboardConsultationDiagnosisComponent implements OnInit {
    doctorData
    activatedtab
    totalPatient
    totalWalkinPatient
                        ...
                        if(result.data[i].total_sales[j].precription.investigationArray){
                            for (var k = 0; k < result.data[i].total_sales[j].precription.investigationArray.length; k++) {
                                for (var l = 0;l < result.data[i].total_sales[j].precription.investigationArray[k].values.length; l++) {
                                    var boolean = false
                                    for (var a = 0; a < this.topProfitArray.length; a++) {
                                        if(result.data[i].total_sales[j].precription.investigationArray[k].name == this.topProfitArray[a].type &&
                                            result.data[i].total_sales[j].precription.investigationArray[k].values[l].display==this.topProfitArray[a].name){
                                            boolean = true
                                        }
                                    }
                                    if (boolean == false) {
                                    ...
                                    for (var l = 0;l < result.data[i].total_sales[j].precription.investigationArray[k].values.length; l++) {
                                        if(result.data[i].total_sales[j].precription.investigationArray[k].name == this.topProfitArray[a].type &&
                                            ...
                                            if(result.data[i].total_sales[j].investigation_cost){
                                                this.topProfitArray[a].sales += result.data[i].total_sales[j].investigation_cost                                            
                        ...
                        for (var j = 0; j < result.data1[i].tests.length; j++) {
                            for (var k = 0; k < result.data1[i].tests[j].amount.length; k++) {
                                if(result.data1[i].tests[j].opt[k] == this.topProfitArray[a].type &&
                                    result.data1[i].tests[j].product[k]==this.topProfitArray[a].name){
                                    this.topProfitArray[a].occurence++
                                    this.topProfitArray[a].sales+=result.data1[i].total_amount
                                    this.topProfitArray[a].percentage = (this.topProfitArray[a].sales*100)/(this.totalWalkinSales+this.totalPharmacySales)
                                }
                            }
                        }

...
@Component({
	...
	templateUrl: './pharmacyprogress.component.html',
	styleUrls: ['./pharmacyprogress.component.css'],
	providers: [UserService]
})
export class PharmacyprogressComponent implements OnInit {
	headerClicked
	pharmacyData
	totalPrescriptions
	totalSales
	...
	getDocData(){
		...
		this.loading = true
		this.userService.PharmacyProgressToday(ob).subscribe(data => {
			console.log(data)
    ...
    SendEmailToAdminDoctorForPharmacyProgressToday(){
    	for (var i = 0; i < this.dataArray.length; i++) {
		...
		console.log(ob)
		this.loading = true
		this.userService.SendEmailToAdminDoctorForPharmacyProgressToday(ob).subscribe(data => {
			console.log(data)
			if(data.response){

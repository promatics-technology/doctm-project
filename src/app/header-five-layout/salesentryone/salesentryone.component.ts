...
@Component({
  ...
  templateUrl: './salesentryone.component.html',
  styleUrls: ['./salesentryone.component.css'],
  providers: [UserService]
})
export class SalesentryoneComponent implements OnInit {
    ...
    saveSalesDetails() {
        ...
        console.log(ob)
        this.loading = true
        this.userService.CreateSalesEntryWalkin(ob).subscribe(data => {
            this.loading = false
            console.log(data)
            if (data.response == true) {
                this.toastr.success(data.message, "Success")
